...
use Modules\SaaS\Services\SaaSManager;
...
if (! function_exists('get_options')) {
    /**
     * Get all theme options from database with caching
     * 
     * Cache is automatically tenant-aware:
     * - DatabaseTenancyBootstrapper switches DB connection to tenant
     * - DatabaseCacheTenancyBootstrapper adds tenant prefix to cache keys
     * - Cache stores in tenant's database with prefixed keys
     * 
     * IMPORTANT: Always use the same cache key 'options' - the tenant prefix
     * is added automatically by the bootstrapper.
     */
    function get_options($key = 'options')
    {
        // For non-tenant contexts during bootstrap, fetch directly from DB
        try {
            $tenancy = app(\Stancl\Tenancy\Tenancy::class);
            if (!$tenancy->initialized && !is_central_domain()) {
                return ThemeSetting::all()->keyBy('key');
            }
        } catch (\Exception $e) {
            // If tenancy not available (e.g., during bootstrap), fetch from DB
            return ThemeSetting::all()->keyBy('key');
        }
...
if (! function_exists('get_translations')) {
    ...
    {
        if (!is_tenant_context() && !is_central_domain()) {
...
if (! function_exists('translate')) {
    ...
    function translate($key, $value = null, $lang = null, $addslashes = false)
    {
        ...
        if (! isset($runtimeCache[$lang])) {
            $translations = [];
            if (app()->bound('translations')) {
            ...
            $runtimeCache[$lang] = $translations;
        }

        $translations = $runtimeCache[$lang];
        ...
        if (!isset($translations[$lang_key])) {
            ...
            $translations[$lang_key] = $value ?: $key;
            $runtimeCache[$lang] = $translations;
            if (app()->bound('translations')) {
                app()->instance('translations', $translations);
            }
            // Refresh cache
...
if (! function_exists('is_central_domain')) {
    function is_central_domain()
    {
        ...
        return in_array($requestHost, $centralDomains);
    }
}
...
if (!function_exists('saas_enabled')) {
     ...
     * @return bool
     */
    function saas_enabled(): bool
    {
        if (!class_exists(SaaSManager::class) || !app()->bound(SaaSManager::class)) {
            return false;
        }

        $saasManager  = app(SaaSManager::class);
...
if (!function_exists('edulab_asset')) {
    /**
     * Generate asset URL - works for both SaaS and non-SaaS setups
     * Always returns central public assets (CSS, JS, images from public/)
     * 
     * @param string $path Path to asset in public directory
     * @param bool $secure Use HTTPS
     * @return string
     */
    function edulab_asset(string $path, bool|null $secure = null): string
    {
        // For central public assets, always use the base asset() without tenant context
        // This works because these are static files in public/, not uploaded files
        return is_tenant_context() ? asset($path, $secure) : asset('storage/' . $path, $secure);
    }
}

if (!function_exists('edulab_global_asset')) {
    /**
     * Generate asset URL - works for both SaaS and non-SaaS setups
     * Always returns central public assets (CSS, JS, images from public/)
     * 
     ...
     * @return string
     */
    function edulab_global_asset(string $path, bool|null $secure = null): string
    {
        // For central public assets, always use the base asset() without tenant context
        // This works because these are static files in public/, not uploaded files
        return is_tenant_context() ? global_asset($path, $secure) : asset($path, $secure);
    }
}

if (!function_exists('theme_asset')) {
    /**
     * Get theme-specific uploaded asset (logo, favicon, etc.)
     * These can be tenant-specific or central depending on context
     * 
     * @param string $path Path within storage/lms/theme-options/
     * @return string
     */
    function theme_asset(string $path): string
    {
        return edulab_asset('lms/theme-options/' . $path);
    }
}
