@php
    ...
    $bgImage =
        $user->cover_photo && fileExists('lms/organizations', $user->cover_photo) == true
            ? edulab_asset("lms/organizations/{$user->cover_photo}")
            : edulab_global_asset('lms/frontend/assets/images/870x260.svg');

    $imgSrc =
        $profile_img && fileExists('lms/organizations', $profile_img) == true
            ? edulab_asset("lms/organizations/{$profile_img}")
            : edulab_global_asset('lms/frontend/assets/images/870x260.svg');
        ...
        <div class="flex-center !justify-start flex-col px-6 pb-10 w-full -mt-10 gap-2 grow">
            <div class="relative">
                ...
                @if ($organization->is_verify)
                    <img data-src="{{ edulab_global_asset('lms/frontend/assets/images/icons/verified.svg') }}" alt="Verified Icon"
