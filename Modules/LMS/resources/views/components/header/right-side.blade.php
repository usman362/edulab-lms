...
@if ($wishlist['is_show'] ?? true)
    <a href="{{ $wishlistUrl ?? '#' }}" aria-label="Wishlist icon"
        ...
        <img data-src="{{ $wishlist['icon_image'] ?? edulab_global_asset('lms/frontend/assets/images/icons/wish-list.svg') }}"
            alt="Icon">
        <span
...
@if ($cart['is_show'] ?? true)
    <a href="{{ $cart['url'] ?? route('cart.page') }}" aria-label="Cart icon"
        ...
        <img data-src="{{ $cart['icon_image'] ?? edulab_global_asset('lms/frontend/assets/images/icons/cart.svg') }}" alt="Icon">
        <span
