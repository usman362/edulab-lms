<div class="relative w-full h-[130px]">
    @php
        ...
        $coverImg =
            !empty($userInfo?->cover_photo) && fileExists( 'storage/' . $storagePath, $userInfo->cover_photo)
                ? edulab_asset($storagePath . '/' . $userInfo->cover_photo)
                : edulab_global_asset($defaultCoverImage);

        $profileImg =
            !empty($userInfo?->profile_img) && fileExists('storage/' . $storagePath, $userInfo->profile_img)
                ? edulab_asset($storagePath . '/' . $userInfo->profile_img)
                : edulab_global_asset($defaultProfileImage);

    @endphp

    <img src="{{ $coverImg }}" alt="user-img" class="size-full object-cover">
    <div
        ...
        <img src="{{ $profileImg }}" class="size-full object-cover" alt="user-img">
    </div>
</div>
