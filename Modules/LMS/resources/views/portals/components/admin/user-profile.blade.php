@php
    ...
    $coverImg =
        $userInfo?->cover_photo && fileExists( 'storage/' . $storagePath, $userInfo->cover_photo)
            ? edulab_asset($storagePath . '/' . $userInfo->cover_photo)
            : edulab_global_asset($defaultCoverImage);

    $profileImg =
        $userInfo?->profile_img && fileExists('storage/' . $storagePath, $userInfo->profile_img)
            ? edulab_asset($storagePath . '/' . $userInfo->profile_img)
            : edulab_global_asset($defaultProfileImage);
...
<div class="card p-0 bg-white dark:bg-dark-card rounded-15 overflow-hidden dk-theme-card-square">
    <div class="relative w-full h-[150px]">
        ...
        <img src="{{ $coverImg }}" alt="cover-image" class="size-full object-cover">

        <label style="background:url({{ $profileImg }}); background-repeat:no-repeat ;background-size:cover"
        ...
        <div class="py-5">
            <div class="flex-center">
                <h4 class="text-[22px] leading-none text-heading dark:text-white font-semibold relative">
                    {{ $name }}
                    <img src="{{ edulab_global_asset('lms/assets/images/icons/verified-icon-green.svg') }}" alt="verified-icon"
