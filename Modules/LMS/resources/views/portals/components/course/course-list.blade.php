@php
    $editRoute = 'course.edit';
    $deleteRoute = 'course.destroy';
    $restoreRoute = 'course.restore';
                    ...
                    <tr>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    @if (count($instructors) > 0)
                                        <div class="flex items-center gap-2">
                                            <p class="font-normal text-xs text-gray-900">{{ translate('Instructors') }}
                                                -
                        ...
                        <td class="px-3.5 py-4">
                            @if ($course->status == Modules\LMS\Enums\CourseStatus::PENDING)
                                <div class="badge b-solid badge-primary-solid"> {{ translate('Pending') }}</div>
                            @elseif ($course->status == Modules\LMS\Enums\CourseStatus::APPROVED)
                                <div class="badge b-solid badge-secondary-solid"> {{ translate('Approved') }}</div>
                            @else
                                <div class="badge b-solid badge-danger-solid"> {{ translate('Rejected') }}</div>
                            @endif
                        </td>
                        @if ($isAdmin)
                            <td class="px-3.5 py-4">
                                <select name="status" class="select-status-change form-input form-select"
                                    data-action="{{ route('course.status', $course->id) }}">
                                    <option value="Pending"
                                        {{ $course->status == Modules\LMS\Enums\CourseStatus::PENDING ? 'selected' : '' }}>
                                        {{ translate('Pending') }}</option>
                                    <option value="Rejected"
                                        {{ $course->status == Modules\LMS\Enums\CourseStatus::REJECTED ? 'selected' : '' }}>
                                        {{ translate('Rejected') }}</option>
                                    <option value="Approved"
                                        {{ $course->status == Modules\LMS\Enums\CourseStatus::APPROVED ? 'selected' : '' }}>
                        ...
                        <td class="px-3.5 py-4">
                            <div class="flex items-center gap-1">
                                <a href="{{ route('course.translate', ['id' => $course->id, 'locale' => app()->getLocale()]) }}"
                                ...
                                @if ($course->trashed())
                                    ...
                                    @if ($isAdmin)
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            ...
                                            data-title="{{ $title }}" data-text="Be care!" data-type="admin">
...
@else
    <div class="card flex-center">
        <h2 class="card-title">{{ translate('You have no Course') }}.</h2>
    </div>
@endif
