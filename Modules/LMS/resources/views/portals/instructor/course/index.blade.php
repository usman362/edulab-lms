@php
    $editRoute = 'instructor.course.edit';
    $deleteRoute = 'instructor.course.destroy';
    $restoreRoute = 'instructor.course.restore';
    $translateRoute = 'instructor.course.translate';

    $textTitle = translate('Do you want to move to Trash');
...
<x-dashboard-layout>
    ...
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="My Courses" page-to="Course"
        action-route="{{ route('instructor.course.create') }}" />

    <x-portal::course.filter isInstructor="{{ $isInstructor }}" />

    <div class="card">

        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('instructor.course.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('instructor.course.index', ['filter' => 'trash']) }}"
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($courses as $course)
                            @php
                                ...
                                $thumbnail =
                                    fileExists($folder = 'lms/courses/thumbnails', $fileName = $course->thumbnail) ==
                                    true
                                        ? edulab_asset("lms/courses/thumbnails/{$course->thumbnail}")
                                        : edulab_global_asset('lms/assets/images/placeholder/thumbnail612.jpg');

                                if ($course->trashed()) {
                                    $textTitle = translate('Are you sure you want to delete this permanently');
                                    $text = translate(
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        @else
                                            ...
                                            <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                                ...
                                                data-title="{{ $textTitle }}" data-text="{{ $text }}">
