<x-dashboard-layout>
    <x-slot:title> {{ translate('Manage Blog') }} </x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Blog List" page-to="Blog" action-route="{{ route('blog.create') }}" />

    <div class="card overflow-hidden">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('blog.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('blog.index', ['filter' => 'trash']) }}"
        ...
        @if ($blogs->count() > 0)
                            ...
                            @php
                                ...
                                $thumbnail =
                                    $blog->thumbnail && fileExists('lms/blogs', $blog->thumbnail) == true
                                        ? edulab_asset('/lms/blogs/' . $blog->thumbnail)
                            ...
                            <tr>
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3.5">
                                        ...
                                        @if ($title)
                                            <h6
                                                class="leading-none text-heading dark:text-white font-semibold line-clamp-1">
                                                {{ str_limit($title, 50, ' ...') }}
                                            </h6>
                                        @endif
                                    </div>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if ($blog->trashed())
                                            <button data-action="{{ route('blog.restore', ['id' => $blog->id]) }}"
                                        ...
                                        @else
                                            <a href="{{ route('blog.translate', ['id' => $blog->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button data-action="{{ route('blog.destroy', $blog->id) }}"
            ...
            {{ $blogs->links('portal::admin.pagination.paginate') }}
        @else
            <x-portal::admin.empty-card title="No Blog" action="{{ route('blog.create') }}" btnText="Add New" />
        @endif
    </div>
</x-dashboard-layout>
