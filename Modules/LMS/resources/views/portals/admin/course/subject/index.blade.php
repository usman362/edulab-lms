<x-dashboard-layout>
    <x-slot:title>{{ translate('Manage Subject') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Subject List" page-to="Subject" action-route="{{ route('subject.create') }}" />
    ...
    <div class="card overflow-hidden">
        ...
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            ...
            <a href="{{ route('subject.index', ['filter' => 'trash']) }}"
                ...
                <table
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($subjects as $subject)
                            @php
                                $translations = parse_translation($subject);
                                $image =
                                    $subject->image && fileExists('lms/subjects', $subject->image) == true
                                        ? edulab_asset("lms/subjects/{$subject->image}")
                                        : edulab_global_asset('lms/assets/images/placeholder/thumbnail612.jpg');

                                $title = translate('Do you want to move to Trash');
                                $text = translate('If you move to Trash, the subject data will be trashed.');

                                if ($subject->trashed()) {
                                    $title = translate('Are you sure you want to delete this permanently');
                                    $text = translate(
                                        'If you delete it, the subject data will be permanently removed.',
                                    );
                                }
                            @endphp
                            <tr>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h6 class="leading-none text-heading dark:text-white font-semibold">
                                                <a
                                                    href="#">{{ $translations['name'] ?? ($subject->name ?? '') }}</a>
                                            </h6>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @if ($subject->trashed())
                                            <button data-action="{{ route('subject.restore', $subject->id) }}"
                                        ...
                                        @else
                                            <a href="{{ route('subject.translate', ['id' => $subject->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            data-action="{{ route('subject.destroy', $subject->id) }}"
...
@else
    <x-portal::admin.empty-card title="No Subject" action="{{ route('subject.create') }}" btnText="Add New" />
    @endif
</x-dashboard-layout>
