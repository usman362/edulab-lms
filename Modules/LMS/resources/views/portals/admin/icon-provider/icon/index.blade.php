<x-dashboard-layout>
    <x-slot:title>{{ translate('Manage Icon') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb back-url="{{ route('icon.index') }}" title="Icon" page-to="Icon"
        action-route="{{ route('icon.create') }}" />
    <div class="card overflow-hidden">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('icon.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('icon.index', ['filter' => 'trash']) }}"
        ...
        @if ($icons->count() > 0)
                            ...
                            <tr>
                                    ...
                                    <div class="form-check form-switch">
                                        <label class="inline-flex items-center me-5 cursor-pointer">
                                            <input type="checkbox" class="hidden appearance-none peer status-change"
                                                name="status" data-action="{{ route('icon.status', $icon->id) }}"
                                                {{ $icon->status == 1 ? 'checked' : '' }}
                                                {{ $icon->trashed() ? 'disabled' : '' }}>
                                ...
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @endphp
                                        @if ($icon->trashed())
                                            <button data-action="{{ route('icon.restore', $icon->id) }}"
            ...
            <!-- Start Pagination -->
            {{ $icons->links('portal::admin.pagination.paginate') }}
        @else
            <x-portal::admin.empty-card title="No Icon" action="{{ route('icon.create') }}" btnText="Add New" />
        @endif
    </div>
</x-dashboard-layout>
